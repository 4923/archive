...
간선 또는 관계 유형 $\tau$, ($u$, $\tau$, $v$) $\in$ $\cal{E}$, 그리고 하나의 인접행렬 $A_{\tau}$ 를 간선 종류마다 정의할 수 있다. 이러한 그래프를 *multi-relational* 하다고 말하며 전체의 그래프는 인접 텐서 $\cal{A} \in \mathbb{R}^{\cal{|V| * |R| * |V|}}$ 로 정의된다.[^1] *multi-relational graph*의 두가지 중요한 부분집합은 1. `heterogenous`, 2. `multiplex` 그래프로 나뉜다. *tau* ($\tau$)는 간선의 타입을 의미하며, 간선의 종류에는 위에 기술한 바와 같이 방향이 있는 것, 없는 것, 가중치가 있는 것 등이 포함된다. 이렇게 간선의 종류가 달라지면 Adjacency Matrix $A$도 따로 정의해야 한다.

[^1]: $\cal{A} \in \mathbb{R}^{\cal{|V| * |R| * |V|}}$ : 행렬 A가 실수 집합 V, R의 계량수^[cardinality, cardinal number]로 결정되는 차원으로 구성된다.

**Heterogeneous graph**

Heterogeneous graph에서, 정점들은 *type*에 물들어있다. 다시 말해, 정점 집합의 일부는 다음과 같이 해체될 수 있다: $\cal{V = V_1} \cup \cal{V_2} \cup \dots \cup \cal{V_k}$ where $\cal{V_i} \cap \cal{V_j} = \emptyset, \forall_i \neq j$ [^1]. heterogeneous graph의 간선은 일반적으로 제한이 걸려있는데 이는 집합 $\cal{V}$가 서로 겹치지 않는 부분집합 $\cal{V_1}$ 부터 $\cal{V_k}$^[$\forall$ : forall] 의 합집합이다.

[^1]: $\cal{V = V_1} \cup \cal{V_2} \cup \dots \cup \cal{V_k}$ where $\cal{V_i} \cap \cal{V_j} = \emptyset, \forall_i \neq j$ 
